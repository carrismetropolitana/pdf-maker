<svg
	xmlns="http://www.w3.org/2000/svg"
	xmlns:xlink="http://www.w3.org/1999/xlink"
	x="0"
	y="0"
	enable-background="new 0 0 50 50"
	version="1.1"
	viewBox="0 0 50 50"
	xml:space="preserve"
>
	<symbol id="Comercio" viewBox="-5.7 -5.7 11.3 11.3">
		<path
			fill="#FD0"
			d="M0-5.7c-3.1 0-5.7 2.5-5.7 5.7 0 3.1 2.5 5.7 5.7 5.7 3.1 0 5.7-2.5 5.7-5.7 0-3.1-2.6-5.7-5.7-5.7"
		></path>
		<path
			d="M3.1 1.5c0 .1-.1.1-.2.1h-5l-.2 1.1c0 .1-.1.2-.2.2h-1v-.5h.8l.8-3.9c0-.1.1-.2.2-.2h4c.1 0 .2.1.2.2l.6 2.8c.1.1.1.1 0 .2M2.5.2H1.3l.1.9h1.3L2.5.2zM-.2.2l-.1.9H1L.9.2H-.2zm1-.4l-.1-1h-.9l-.1 1H.8zM-.8 1.1l.1-.9h-1.1l-.2.9h1.2zm-.9-1.3h1l.1-1h-.9l-.2 1zm3.9-1h-1l.1 1h1.1l-.2-1zM-1.5-1.8c-.4 0-.8-.4-.8-.8s.4-.8.8-.8.8.4.8.8c.1.4-.3.8-.8.8m0-1.2c-.2 0-.4.2-.4.4s.2.4.4.4.4-.2.4-.4-.2-.4-.4-.4M1.7-1.8c-.4 0-.8-.4-.8-.8s.4-.8.8-.8.8.4.8.8-.3.8-.8.8m0-1.2c-.2 0-.3.2-.3.4s.2.4.4.4.4-.2.4-.4c-.1-.2-.3-.4-.5-.4"
		></path>
	</symbol>
	<use
		width="11.3"
		height="11.3"
		x="-5.7"
		y="-5.7"
		overflow="visible"
		transform="matrix(4.2463 0 0 -4.2463 25 25)"
		xlink:href="#Comercio"
	></use>
</svg>
