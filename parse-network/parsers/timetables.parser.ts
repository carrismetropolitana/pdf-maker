...
export default async () => {
	...
	// const feedStartDateRaw = await NETWORKDB.client.query<{ feed_start_date: `${number}` }>('SELECT feed_start_date FROM feed_info').then(res => res.rows[0].feed_start_date);
	// const feedStartDate = feedStartDateRaw.slice(6, 8) + '.' + feedStartDateRaw.slice(4, 6) + '.' + feedStartDateRaw.slice(0, 4) as `${number}.${number}.${number}`;
	...
	async function processLineStopPair(LINE_ID: string, STOP_ID: string, index: number | string) {
		console.time(`⤷ ${index}/${filteredLineStopPairs.length - 1} -> Line ${LINE_ID} stop ${STOP_ID}`);
    ...
    WHERE
      ...
      AND stop_times.arrival_time IS NOT NULL
		...
		console.timeEnd(`⤷ ${index}/${filteredLineStopPairs.length - 1} -> Line ${LINE_ID} stop ${STOP_ID}`);
	}
   ...
   * 1m 39s for 10000
   */
	const includedLines = env.timetable.includedLines
	const filteredLineStopPairs = includedLines.length>0 ? lineStopPairs.filter(([LINE_ID,_]) => includedLines.includes(LINE_ID)) : lineStopPairs
	for (const index in filteredLineStopPairs) {
		const [
			LINE_ID, STOP_ID,
		] = filteredLineStopPairs[index];
		// 4600/0/040024
		// if (LINE_ID !== '4600' || STOP_ID !== '040024') continue;
		if (env.timetable.excludedLines.includes(LINE_ID)) continue
	...
	let previousKeys = await SERVERDB.client.keys('timetables:*')
	if (previousKeys.length > 0) {
		await SERVERDB.client.del(previousKeys);
	}
